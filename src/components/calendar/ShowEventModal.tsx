...
const ShowEventModal = ({ isOpen, onClose, submitHandler, temEvent, deleteEvent }) => {
    ...
    const [isAllDay, setIsAllDay] = useState(temEvent.isAllDay);
    const [eventDate, setEventDate] = useState(temEvent.eventDate);
    const [eventDateTo, setEventDateTo] = useState(temEvent.eventDateTo);
    const [eventTime, setEventTime] = useState(temEvent.eventTime);
    const [eventDuration, setEventDuration] = useState(temEvent.eventLocation);
    const [eventLocation, setEventLocation] = useState(temEvent.eventLocation);
    const [isRepeating, setIsRepeating] = useState(temEvent.isRepeating);
    const [repeatEvery, setRepeatEvery] = useState(temEvent.repeatEvery);
    const [repeatFrequency, setRepeatFrequency] = useState(temEvent.repeatFrequency);
    const [repeatFrequencyCount, setRepeatFrequencyCount] = useState(temEvent.repeatFrequencyCount);
    const [repeatDays, setRepeatDays] = useState(temEvent.repeatDays);
    const [repeatEndType, setRepeatEndType] = useState(temEvent.repeatEndType);
    const [repeatEndAfter, setRepeatEndAfter] = useState(temEvent.repeatEndAfter);
    const [repeatEndDate, setRepeatEndDate] = useState(temEvent.repeatEndDate);
